# RavenStack — Business Questions and Analysis Plan

This document records the rationale, proposed approach and interpretation limits for the five business questions. See [business_requirements.md](business_requirements.md) for the fictional business scenario and stakeholder roles.
...
Total MRR alone does not identify which sources of growth or loss changed. Reconcile opening MRR, the five movements and ending MRR for each complete month, then compare net additions and growth rates. Examine both the monetary amounts and each movement relative to opening MRR.
...
### Decision relevance and limitations
...
Company-wide churn can hide deterioration in a particular customer group. Use clearly defined segments supported by the data, such as company size or country, and compare churn counts, churn rates and Churned MRR over complete months.

The customer churn rate uses the fixed cohort paying at the opening cutoff: its numerator is the distinct accounts that lose all paid subscriptions at least once during the month. An account that churns and returns still enters this event-based numerator. Assign these accounts to their opening segment and keep the assignment fixed for the period.

Report opening-cohort churn counts, rates and Churned MRR together, then show Churned MRR from accounts outside that cohort separately when reconciling to the company total. Group sizes, recurring revenue exposure and changes in customer mix remain part of the interpretation.
...
This single-month comparison does not establish a trend. Check whether each segment's MRR loss increased during the slowdown and how much it contributed to the change in net additions.
...
This question investigates whether earlier feature usage is associated with later retention. Start with accounts paying at cutoff T, measure feature active days during the preceding 30 complete reporting dates and assess paid status at T + 30 days. Valid usage-event counts provide supporting volume context. Eligible observations require adequate feature-access and tracking coverage and an observable day-30 paid status.

Usage bands remain subject to feature semantics and source validation. Within each feature and observation period, compare groups' endpoint retention, eligible-account counts and subsequent Churned MRR. If relevant, separately examine Contraction MRR among customers who remain paying. An account that leaves and returns by day 30 is retained at the endpoint, so endpoint retention alone does not recover every churn event.
...
Use the latest three complete months in the dataset and attribute each qualifying Expansion event to the plan immediately before the event. Within each plan/month, sum the Expansion amounts by account, count distinct expanding accounts and calculate their average total contribution. Accounts without expansion do not enter this average. Also consider the eligible customer base when interpreting differences.
